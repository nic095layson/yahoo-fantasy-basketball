"""Mock 36 (slot 1, declared triple punt FT%/3PTM/TO) — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock36.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock36.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock36.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock36.json")
state = json.load(open(STATE))
OWNER = state["slot"]
assert state["teams"] == 12 and state["size"] == 13 and OWNER == 1, state
picks = state["picks"]
...
out = {
    "mock": 36, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock36_out.json")
