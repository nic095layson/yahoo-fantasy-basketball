#!/usr/bin/env python3
"""mock51_retro — retro analysis of draft_state_51 (owner slot 10) on the deck plane.

    python3 arena/mocks/mock51_retro.py <stage>

Stages (each writes arena/results/m51_<stage>.json):
  replay     card reconstruction at all 13 owner turns, v22 (drafted-against)
             and v23 (current) pools — exact port of decwScores (check_parity)
  ...
  forecast   the room-forecast experiment: partial-ΔECW vs forecast-ΔECW vs
             oracle-ΔECW orderings, graded by hindsight gain
...
if not os.path.exists(POOLS["v22"]):
    import subprocess
    with open(POOLS["v22"], "w", encoding="utf-8") as _f:
        _f.write(subprocess.run(["git", "-C", DECK, "show", V22_REV + ":data/players.csv"],
                                capture_output=True, text=True, check=True).stdout)
STATE = DECK + "/arena/data/states/draft_state_51.json"
state = json.load(open(STATE, encoding="utf-8"))
TEAMS, SLOT, SIZE = state["teams"], state["slot"], state["size"]
PICKS = state["picks"]
assert TEAMS == 12 and SLOT == 10 and SIZE == 13 and len(PICKS) == 156
OWNER_IDX = [n for n in range(len(PICKS)) if hoops.team_of_pick(n, TEAMS) == SLOT]
assert len(OWNER_IDX) == 13 and all(PICKS[n]["slot"] == SLOT for n in OWNER_IDX)
...
def stage_replay():
    ...
    for tag in ("v22", "v23"):
        ...
        for t in turns:
            ...
            print(f"        top5: {top5}")
    json.dump(out, open(SP + "/m51_replay.json", "w"), indent=1)
    return out
...
def stage_final():
    ...
    json.dump(out, open(SP + "/m51_final.json", "w"), indent=1)
    return out
...
def stage_hindsight(tag="v23"):
    ...
    replay = json.load(open(SP + "/m51_replay.json"))
    cards = {t["pick"]: t for t in replay["v23"]}
    cards22 = {t["pick"]: t for t in replay["v22"]}
    ...
    for n in OWNER_IDX:
        ...
        for i, p in enumerate(c["pool"]):
            ...
            rows.append(dict(n=nm, t=p["team"], pos=p["pos"], ecw=round(e, 4),
                             ...
                             card_rank_v22=next((x["rank"] for x in cards22[n + 1]["top"] if x["n"] == nm), None)))
    ...
    json.dump(out, open(SP + "/m51_hindsight.json", "w"), indent=1)
    return out
...
def stage_forecast(tag="v23", K=12):
    ...
    for n in OWNER_IDX:
        ...
        print(f"#{n + 1:>3} {actual:<22} " + "  ".join(
            ...
            for lab in orders))
    json.dump(out, open(SP + "/m51_forecast.json", "w"), indent=1)
    return out
...
def stage_arms(tag="v23"):
    ...
    for name, sw in arms.items():
        ...
        json.dump(results, open(SP + "/m51_arms.json", "w"), indent=1)
    return results
