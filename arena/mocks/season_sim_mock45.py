"""Mock 45 (slot 8, declared triple punt FT%/3PTM/TO) - standard room — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock45.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock45.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock45.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock45.json")
...
print(f"mock 45 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 45, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock45_out.json")
