"""Mock 37 (slot 2, declared triple punt FT%/TO/3PTM) — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock37.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock37.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock37.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock37.json")
...
out = {
    "mock": 37, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock37_out.json")
