"""Mock 47 (slot 10, punt FT%/3PTM/TO) - standard room — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock47.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock47.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock47.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock47.json")
...
print(f"mock 47 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 47, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock47_out.json")
