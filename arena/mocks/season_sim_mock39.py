"""Mock 39 (slot 4, declared triple punt FT%/3PTM/TO) — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock39.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock39.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock39.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock39.json")
...
print(f"mock 39 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 39, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock39_out.json")
