"""Mock 42 (slot 5, NO punt declared) - standard room, first post-discard mock — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock42.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock42.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock42.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock42.json")
...
print(f"mock 42 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 42, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock42_out.json")
