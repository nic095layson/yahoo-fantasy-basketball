"""Mock 40 (slot 4, declared triple punt 3PTM/TO/FT%) - FIRST v2-INSTRUMENT MOCK — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock40.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock40.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock40.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock40.json")
...
print(f"mock 40 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 40, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock40_out.json")
