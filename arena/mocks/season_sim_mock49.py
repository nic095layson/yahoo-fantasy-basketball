"""Mock 49 (slot 8, punt FT%/3PTM) - standard room — season sim + readouts.
...
arena/data/states/draft_state_mock49.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock49.py`
regenerates every number quoted in the mock-49 debrief and LEDGER row.
"""
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock49.json")
...
print(f"mock 49 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 49, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock49_out.json")
