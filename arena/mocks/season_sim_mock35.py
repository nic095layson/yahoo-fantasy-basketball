"""Mock 35 (slot 12, declared triple punt FT%/AST/3PTM) — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock35.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock35.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock35.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock35.json")
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock35_out.json")
