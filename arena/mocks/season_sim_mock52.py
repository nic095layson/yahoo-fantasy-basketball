"""Mock 52 (slot 12, NO declared punt) - standard room — season sim + readouts.

Written under LESSONS.md lesson 13 (evidence-landing law), mirroring
season_sim_mock48.py:
repo-relative paths only; the draft state lives at
arena/data/states/draft_state_mock52.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock52.py`
regenerates every number quoted in the mock-52 debrief and LEDGER row.
"""

import os as _os_epoch
_os_epoch.environ["ARENA_WEEK_MODEL"] = "daily"  # v2-epoch harness (instrument v2, daily-fill scoring)
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock52.json")
state = json.load(open(STATE))
OWNER = state["slot"]
assert state["teams"] == 12 and state["size"] == 13 and OWNER == 12, state
picks = state["picks"]
...
print(f"mock 52 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 52, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock52_out.json")
