"""Mock 51 (slot 5, punt FT%/3PTM/AST) - standard room — season sim + readouts.
...
arena/data/states/draft_state_mock51.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock51.py`
regenerates every number quoted in the mock-51 debrief and LEDGER row.
"""
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock51.json")
state = json.load(open(STATE))
OWNER = state["slot"]
assert state["teams"] == 12 and state["size"] == 13 and OWNER == 5, state
picks = state["picks"]
...
print(f"mock 51 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 51, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock51_out.json")
