"""Mock 48 (slot 4, punt FT%/3PTM/TO) - standard room — season sim + readouts.
...
arena/data/states/draft_state_mock48.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock48.py`
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock48.json")
...
print(f"mock 48 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 48, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock48_out.json")
