"""Mock 50 (slot 2, punt AST/3PTM/PTS) - standard room — season sim + readouts.
...
arena/data/states/draft_state_mock50.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock50.py`
regenerates every number quoted in the mock-50 debrief and LEDGER row.
"""
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock50.json")
state = json.load(open(STATE))
OWNER = state["slot"]
assert state["teams"] == 12 and state["size"] == 13 and OWNER == 2, state
picks = state["picks"]
...
print(f"mock 50 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 50, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock50_out.json")
