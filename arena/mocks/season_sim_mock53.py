"""Mock 53 (slot 5, NO declared punt) - standard room — season sim + readouts.
...
arena/data/states/draft_state_mock53.json and the output JSON is committed
next to the results. Fresh clone + `python3 arena/mocks/season_sim_mock53.py`
regenerates every number quoted in the mock-53 debrief and LEDGER row.
"""
...
STATE = os.path.join(REPO, "arena", "data", "states", "draft_state_mock53.json")
...
print(f"mock 53 — slot {OWNER}, declared punt {DECLARED_PUNT}")
...
out = {
    "mock": 53, "owner_slot": OWNER, "declared_punt": DECLARED_PUNT,
...
OUT = os.path.join(REPO, "arena", "results", "season_sim_mock53_out.json")
