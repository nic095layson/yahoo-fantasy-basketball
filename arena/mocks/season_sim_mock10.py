...
STATE = "/root/.claude/uploads/58588377-022f-59e5-ac2c-106514acd881/534a7f33-draft_state_10.json"
state = json.load(open(STATE))
assert state["teams"] == 12 and state["size"] == 13 and state["slot"] == 4, state
picks = state["picks"]
...
# Persona map: deck mockCastFor() — MOCK_CAST assigned in slot order, skipping owner slot 4
...
print("\nkept-total rank of slot 4:", sorted(rosters, key=lambda x: -ztot[x]).index(4) + 1)
print("champ rank of slot 4:", order.index(4) + 1, "of 12")
print("per-seed slot4 champ%:", {sd: round(100*per_seed[sd][0][4]/N, 2) for sd in SEEDS})
print("per-seed slot4 playoff%:", {sd: round(100*per_seed[sd][1][4]/N, 2) for sd in SEEDS})

print("\nslot 4 per-category z-sum + rank (1=best; TO inverted so higher=better everywhere):")
ranks4 = {}
for c in CATS:
    ranked = sorted(rosters, key=lambda i: -zsum[i][c])
    ranks4[c] = ranked.index(4) + 1
    print(f"  {c:>5}: rank {ranks4[c]:>2}   z-sum {zsum[4][c]:+7.2f}")

by_rank = sorted(CATS, key=lambda c: (ranks4[c], -zsum[4][c]))
print("\n3 strongest:", [(c, ranks4[c], round(zsum[4][c], 2)) for c in by_rank[:3]])
print("3 weakest:  ", [(c, ranks4[c], round(zsum[4][c], 2)) for c in by_rank[-3:]])
...
out = {
    ...
    "slot4_cat_ranks": ranks4,
    "slot4_cat_z": {c: round(zsum[4][c], 2) for c in CATS},
}
json.dump(out, open("/tmp/claude-0/-home-user-yahoo-fantasy-basketball/58588377-022f-59e5-ac2c-106514acd881/scratchpad/season_sim_mock10_out.json", "w"), indent=1)
print("\nwrote season_sim_mock10_out.json")
