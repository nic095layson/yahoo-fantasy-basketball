...
| E18 | **Named manager profiles (owner directive 2026-08-04):** (a) deck MOCK mode fields the 11 real managers from `arena/profiles.json` — per-manager adp/value weights, noise, position bias, Noah as pure autodraft — shuffled across seats each draft; (b) LIVE mode with named seats replaces the generic room-mix survival with **per-seat pick projection**: P(manager m takes player p before my next turn), chained across the actual managers sitting between my picks, feeding BUY NOW/quiet and a pre-draft per-round target sheet per rival | Three-season boards + complete identity map (league_intel §12); profiles.json committed 2026-08-04 | Mock cast: replay calibration — simulated rooms must reproduce each manager's measured 25-26 reach index within ±8; survival: BUY NOW precision ≥80% maintained; October brief consumes the projection sheet |
