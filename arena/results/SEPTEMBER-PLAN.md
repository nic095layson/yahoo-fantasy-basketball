...
| — | **E22c SUPERSEDED-IN-PART 2026-08-05, same day (owner-directed scoring audit — registered text above unedited): INSTRUMENT v2 SHIPPED.** The owner supplied the league's scoring rules as ground truth (daily lineups; started slots incl. Util count 100%; bench counts 0), confirmed against Yahoo primary docs, and the daily-fill MC (`daily_lineup_mc.py`) measured real effective bench weight ≈0.975 vs the modeled 0.15 (weekly counting totals ran 8–23% low). `team_week_model` and the deck's `teamWeekModel` moved to daily-fill start rates (bit-identical cross-language, 2808/2808 weight parity + 72/72 hash vectors; `ARENA_WEEK_MODEL=static` regenerates all pre-v2 numbers). Ordering consequence: #1 changes at 22/169 panel turns, all R8+; m34's R13 all-big card became 3/5 C with two guards organically. Panel disclosure per E21: `v2_panel_disclosure.py` (baseline / v1cards / v2cards under v2 grading; escalation rule in docstring). REMAINING for September: fit the v2 constants (game-count distribution, day weights, availability tiers) to the owner's real weekly breakdowns — the calibration half of the registered row stands | shipped + disclosure | v2 live; calibration open |
| E25 | **Bot competence upgrade — opt-in "sharp mode" for the 11 named managers (registered 2026-08-06, owner-directed).** Add an anti-dead-cat competence term to `managerScores` at R8+: bonus for categories still within reach of the pack, penalty for adding to an already-dead one. ADP weights, noise, and loyalty stay fitted — personality preserved, only competence moves | Mock 40 room measurement (`findings_2026-08-06_mock40_room_synergy.md`): the 11 bots average **1.64 dead kept categories** and 4.36 live cats vs the owner's 0 and 6; daily-fill lineup waste is negligible for every seat, so the gap is category coherence, not roster construction. Design tension recorded: the bots are FITTED FORECASTS of real humans (E18b Spearman 0.952), so making them stronger makes the room less predictive of the real October draft — hence opt-in, default cast unchanged | **Bar, registered before any run (all three prongs):** (1) sharper — across >=6 fresh mock rooms, mean bot dead-kept-cats <= 0.8 and mean bot ECW +>=0.05/wk; (2) harder — replaying the owner's identical mock-40 draft into a sharp room lowers his champ% by >=3pp; (3) realism guard — default mode byte-identical and the E18b reach gate still passes. Needs a new harness that generates fresh rooms from the deck's own AI |
| — | **E25 MEASURED AND REJECTED 2026-08-06, same day it was registered (dated resolution line; registered text above unedited).** Built the competence term + opt-in SHARP ROOM toggle, generated 8 CRN-paired rooms with the deck's own AI (`room_driver.js`, `e25_measurement.py`), graded 18k seasons/arm on the v2 instrument. **1 of 4 prongs passes:** dead-kept 1.50 (needs <=0.8) FAIL; bot ECW +0.002 (needs +0.05) FAIL; owner champ -2.53pp (needs -3pp) FAIL, and inconsistent (3 of 8 rooms got EASIER); realism guard 8/8 byte-identical PASS. NOT SHIPPED - deck reverted byte-identical. Structural diagnosis, not a tuning miss: a held-out dev-seed sweep moved the metric 0.14 while raising the weight 7.5x, and no round threshold helped - **coherence requires CONCESSION, and the term only ever adds to weak cats.** (`findings_2026-08-06_e25_sharp_mode.md`) | measured | closed; E25b registered below |
| — | **E25 OWNER OVERRIDE 2026-08-06 (disclosure per bar-registry law; the bar above is NOT renegotiated - it still reads FAIL).** Presented with the measured shortfall (owner champ -2.53pp vs the -3pp target, 3 of 8 rooms EASIER, dead-kept 1.50 vs <=0.80, bot ECW +0.002 vs +0.05), the owner elected to keep the toggle as a rough stress test. SHIPPED under explicit labeling, not as a validated feature: the button reads "SHARP ROOM - rough stress test (unvalidated)", its tooltip, the draft log line, and the deck's Logic paragraph each state that it failed its bar and quote the measured -2.5pp/5-of-8 numbers; default MOCK remains the draft-night forecast. Guards re-verified after restore: default-mode rooms 8/8 byte-identical to pre-E25, owner Top-5 card replays identical on m40 and m38 (sharp mode cannot reach the owner's advice), 10/10 UI assertions. E25b remains the real fix | owner decision on an under-bar feature | shipped as labeled stress test |
| — | **E25b MEASURED AND DISCARDED 2026-08-06, same day it was registered (dated resolution line; registered text below unedited). Owner instruction: "If it fails again, discard."** Variant built in the harness only (`mk_e25b_variant.py`; deck never modified), screened on held-out dev seeds (declare-after-R3 was the only setting beating default), then run on the registered 8 rooms, 18k CRN-paired seasons/arm. **FAILED all three measurable prongs, prong 2 REVERSED:** bot dead-kept 1.57 (needs <=0.80) FAIL; bot ECW -0.007 (needs +0.05) FAIL - the bots got WORSE; owner champ **+9.28pp** (needs -3pp) FAIL - the room got EASIER in **8 of 8 rooms**. **Finding: G1a generalizes from the owner's card to the bots** - punt-DECLARING cost -5 to -11pp for the owner in July and costs +9.28pp in the mirror here; conceding on round-3 evidence and compounding it for ten rounds is not judgment, it is a smaller pool. With E25 (too weak, -2.53pp) the two results bracket the problem: bot category coherence is reachable by neither lever, and NO further follow-up is registered - three measured attempts is enough. Reachability note: the 0.80 target was anchored on one observation (mock 40 owner = 0); the owner seat following the shipped card averages 1.12 across these rooms, so prong 1a asked bots to beat the tool's own policy (`findings_2026-08-06_e25b_frame_bots.md`) | measured | discarded; no successor |
| — | **E25 SHARP MODE DISCARDED 2026-08-06 (owner instruction, superseding the same-day override).** After E25b also failed, the owner directed "Discard sharp mode". Removed from the deck in full - engine term, toggle, state flag, log line, export field and the Logic sentence; verified by 8/8 byte-identical room generation against the pre-E25 deck. Both measurement records and harnesses are retained (`e25_measurement_out.json`, `e25b_measurement_out.json`, `mk_e25b_variant.py`). Mock 41 was drafted in a sharp room before the removal and is marked accordingly in the LEDGER | owner instruction | closed; deck clean |
| E25b | **Frame-declaring bots (registered 2026-08-06, from the E25 diagnosis).** After R3-R4, each manager picks the 3-cat punt its roster best supports and drafts to it for the rest of the draft, conceding those cats outright - the concession E25's additive term structurally could not produce | E25: no (weight, round-threshold) setting brings bot dead-kept-cats below ~1.34 vs the 0.8 target; the owner's own five-draft run shows coherence comes from declaring 3 cats dead in R1-R7 | **Same bar text as E25** (dead-kept <=0.8; bot ECW >=+0.05/wk; owner champ <=-3pp; realism guard byte-identical). Counterweight to measure rather than assume: G1a put punt-DECLARING at -5 to -11pp for the OWNER's card; whether that transfers to bots is open - if declared frames make bots weaker, that is the answer and it gets written up |
