...
1. **Before the draft** — the user may say **"Prime for live draft"**: run the full pre-draft checklist in one shot — freshness status (refresh news + stamp if stale), pool validation, `draft init` with the user's slot, and confirm the response protocol (mirror lines, confidence boards, SYNC/RESYNC) in a two-line ready message. Otherwise: confirm the user's slot and the exact roster-slot layout, and whether they're committed to a punt build (offer `rank --punt` comparisons if undecided). Web-search for injury updates on likely first-two-round picks. **Consult arena intel if present**: `arena/results/slot_intel.json` (which builds win from the user's slot) and `arena/results/cadence_intel.json` (expected tier/position drain between the user's turns — reach when the cadence says a tier won't survive the wait; wait when it will). Then `python3 scripts/hoops.py draft init --teams 12 --size 15 --slot K [--punt ...]`.
