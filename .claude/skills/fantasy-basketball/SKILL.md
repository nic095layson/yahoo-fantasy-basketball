...
7. **Corrections**: `draft undo` reverses the last pick (any team's); earlier picks are patched directly in draft_state.json (picks is an ordered list of {player, slot}).
8. If a drafted player isn't in the pool (deep-league picks), say so, log nothing, and continue — but note the skipped pick means snake attribution is off by one from then on; log remaining picks with explicit `--slot`.
