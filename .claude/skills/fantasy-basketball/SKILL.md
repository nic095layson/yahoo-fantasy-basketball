...
5. **Corrections and ambiguity (owner's rule, learned 2026-07-11)**: a re-sent already-drafted name in a bulk feed logs an UNKNOWN placeholder (numbering preserved) — NEVER infer it is a correction to the earlier pick; corrections apply only via explicit pick numbers ("44- Herro" or `draft fix`), echoed before/after. Ambiguous names auto-resolve in the engine to the best available candidate with an "(assumed over ...)" flag — relay the flag, don't stall. Typos fuzzy-match automatically (Siakim→Siakam).
